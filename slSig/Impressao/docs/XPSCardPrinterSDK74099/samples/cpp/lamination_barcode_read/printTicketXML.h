...
namespace PrintTicketXml
{
    ...
    //
    // Forming an action XML string -->
    // e.g., L1 both sides:
    // CString actionXML = FeatureNamePrefix
    //                      +JobLamination1FeatureName
    //                      +LaminationActionBothSides
    //                      +FeatureNameSuffix;

    const CString FeatureNamePrefix("<psf:Feature ");
    const CString FeatureNameSuffix("</psf:Feature>");

    const CString JobLamination1FeatureName(
        "name=\"ns0000:JobLamination1Options\">");
    const CString JobLamination2FeatureName(
        "name=\"ns0000:JobLamination2Options\">");

    const CString LaminationActionDoNotApply(
        "  <psf:Option name=\"ns0000:Donotapply\" />");
    const CString LaminationActionSide1(
        "  <psf:Option name=\"ns0000:Side1\" />");
    const CString LaminationActionSide2(
        "  <psf:Option name=\"ns0000:Side2\" />");
    const CString LaminationActionBothSides(
        "  <psf:Option name=\"ns0000:BothSides\" />");
    const CString LaminationActionSide1twice(
